...
| Field                | Bits | Ghi chú ngắn |
|---------------------|-----:|--------------|
| **Version**         | 4    | Luôn = **4** cho IPv4. |
| **IHL**             | 4    | Internet Header Length = số **word 32-bit** của header. **Min 5** (20B), **Max 15** (60B). |
| **DSCP**            | 6    | QoS đánh dấu mức ưu tiên (Differentiated Services). |
| **ECN**             | 2    | Explicit Congestion Notification. |
| **Total Length**    | 16   | **Header + Data** (byte). Tối đa **65,535**. |
| **Identification**  | 16   | ID dùng cho **phân mảnh** (fragments) cùng gói. |
| **Flags**           | 3    | bit: **RB**(0), **DF**(Don’t Fragment), **MF**(More Fragments). |
| **Fragment Offset** | 13   | Vị trí mảnh (đơn vị **8 byte**). |
| **Time To Live**    | 8    | Số hop tối đa; giảm 1 mỗi router; 0 ⇒ drop. |
| **Protocol**        | 8    | Lớp trên: **1**=ICMP, **6**=TCP, **17**=UDP, **89**=OSPF… |
| **Header Checksum** | 16   | Checksum **chỉ** cho phần header (router cập nhật TTL ⇒ tính lại). |
| **Source Address**  | 32   | IPv4 nguồn. |
| **Destination Address** | 32 | IPv4 đích. |
| **Options**         | ≤320 | Tối đa **40 byte** (320 bit). Ít dùng; khi có ⇒ IHL > 5. |
